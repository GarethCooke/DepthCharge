...
| D7 | B | **HUB75 switch-on brownouts the DevKit — mechanism OPEN, and the two candidates want different remedies** | 2026-08-24, at the bench: switching panel power ON while the board was running produced `rst:0xf (BROWNOUT_RST)` after 35.8 h of uptime. Switching it OFF ten minutes later did **not** — uptime stayed monotonic — so the event is **inrush-driven and one-directional**. `hardware/BRINGUP.md:28,36` records the rails as **not tied** (panel on the LRS-50-5, DevKit on USB, common GND only), so *"give the panel its own rail"* is already the case and is a no-op remedy. Two candidates remain and they diverge: **(a)** a transient on the shared ground return as several amps of inrush flow through it — remedy is bonding/series impedance; **(b)** injection through the HUB75 lines and the 74HCT245s while the panel side comes up — remedy is sequencing or series resistors. **The discriminator is one minute**: power the panel with the ribbon DISCONNECTED. Reboot ⇒ ground return; no reboot ⇒ signal path. **It is only valid if the ground bond is the separate `CH1−` strap and not the ribbon's own GND pins** — if the ribbon is the only bond, pulling it removes both candidate paths and neither hypothesis is distinguished, which is the coincidence rule. Cheapest to run when the board's counters have just reset anyway. Feeds M7's carrier board. |
