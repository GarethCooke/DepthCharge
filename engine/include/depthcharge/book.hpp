...
public:
    ...
    void publish(DisplaySnapshot& out) noexcept {
        ...
        out.bid_count = copy_levels(bids_, bid_count_, out.bids);
        out.ask_count = copy_levels(asks_, ask_count_, out.asks);

        // Ring -> newest-first array. trade_head_ is the *next* write slot, so
        // the newest print sits one behind it, modulo the ring.
        ...
        for (std::uint32_t i = n; i < ring; ++i) { out.trades[i] = TradePrint{}; }
...
private:
    ...
    void adopt(const FeedEvent& ev) noexcept {
        bid_count_ = copy_span(ev.bids, bids_);
        ask_count_ = copy_span(ev.asks, asks_);
    ...
    static std::uint32_t copy_span(const LevelSpan& src, BookLevel* dst) noexcept {
        const std::uint32_t n =
            src.size < kBookCapacity ? src.size : static_cast<std::uint32_t>(kBookCapacity);
        for (std::uint32_t i = 0; i < n; ++i) { dst[i] = src.data[i]; }
        return n;
    }

    static std::uint8_t copy_levels(const BookLevel* src, std::uint32_t count,
                                    BookLevel* dst) noexcept {
        const std::uint32_t n =
            count < kDisplayLevels ? count : static_cast<std::uint32_t>(kDisplayLevels);
        for (std::uint32_t i = 0; i < n; ++i) { dst[i] = src[i]; }
        for (std::uint32_t i = n; i < kDisplayLevels; ++i) { dst[i] = BookLevel{}; }
